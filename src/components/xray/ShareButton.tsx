...
export function ShareButton({ agent }: { agent: AgentSummary }) {
  ...
  const secondaryClass =
    'border border-border bg-surface px-3 py-1.5 text-xs font-mono text-foreground-secondary hover:bg-surface-hover hover:text-foreground hover:border-border-bright transition-colors'
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 flex flex-col border border-border-bright bg-surface shadow-lg min-w-[200px]">
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => setOpen(false)}
            className="px-4 py-2.5 text-xs font-mono text-foreground-secondary hover:bg-surface-hover hover:text-foreground transition-colors text-left border-t border-border"
          >
            I Own This Agent
          </a>
        </div>
